...
ssh $mtuser@$mtip -p $mtsshport "/interface/wireguard/peers/add interface=$interface preshared-key=\"$presharedkey\" comment=$username allowed-address=$address public-key=\"$publickey\""
